...
"""This function searches through a word 'guess' for a specific character 'char'. If 
the word contains the character, it returns true. If not, false."""
...
def input_guess(num_char: int) -> str:
    ...
    while len(guess) != num_char:
        guess: str = input(f"That wasn't { num_char } chars! Try again: ")
    return guess
...
def main() -> None:
    ...
    game_won: bool = False
    secret: str = "max"
    while turn <= 6 and game_won == False:
        print(f"=== Turn { turn }/6 ===")
